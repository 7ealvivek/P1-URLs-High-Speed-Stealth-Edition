#!/usr/bin/env python3
"""
P1-URLs.py - v4.0.0 (The Final Stable Release)

This is the definitive, stable version of the P1-URLs scanner. It has been
manually reviewed and refactored to correct all known bugs, including persistent
syntax, name, import, and indentation errors. All features are stable and
production-ready.
"""
...
# Use a try-except block for backward compatibility with the 'rich' library
try:
    from rich.panel import Panel
except ImportError:
    # This is the correct fallback for older system versions
...
def print_banner():
    ...
    for line in ascii_art_lines:
        ...
        time.sleep(0.05)
    subtitle_text = "\n[yellow]v3.0.0 Final Production Release[/yellow]\n"
...
def run_uro(input_file, output_file):
    ...
    if not raw_urls.strip():
        console.print("[yellow][-] No URLs to deduplicate.[/yellow]"); Path(output_file).touch(); return 0
...
def run_gf(input_file, gf_output_dir, basefile):
    ...
    if not Path(input_file).exists() or Path(input_file).stat().st_size == 0:
        console.print("[yellow][-] No URLs to classify.[/yellow]"); return
    with open(input_file) as f:
...
def test_lfi_dynamically(lfi_urls_file, args, lfi_results_file, concurrency):
    ...
    if args.lfi_payloads:
        try:
            ...
            console.print(f"[red][!] Custom LFI payload file not found at {args.lfi_payloads}. Using defaults.[/red]")
    ...
    for i, url in enumerate(urls_to_test):
        ...
        try: base_resp = make_throttled_request("GET", url, headers=CLEAN_BASE_HEADERS, timeout=REQUEST_TIMEOUT, verify=False); base_content = base_resp.text
        ...
        for param in query_params:
            ...
            test_cases_for_param = [(session, base_content, param, payload, query_params, parsed_url) for payload in lfi_payloads]
                ...
                for future in pbar:
                    ...
                    try:
                        if (result := future.result()) and not found:
                            ...
                            found = True; [f.cancel() for f in futures]
...
def _check_sqli_vulnerability_time(target_url, method, headers):
    try:
        start_baseline = time.time(); make_throttled_request("GET", urlparse(target_url)._replace(query="").geturl(), headers=CLEAN_BASE_HEADERS, timeout=5, verify=False); baseline_duration = time.time() - start_baseline
        start_attack = time.time(); make_throttled_request(method.upper(), target_url, headers=headers, data={"p1":"p1"} if method.upper() in ["POST", "PUT", "PATCH"] else None, timeout=(TIME_DELAY + 5), verify=False); attack_duration = time.time() - start_attack
...
def test_blind_sqli(sqli_urls_file, blind_sqli_results_file, oob_log_file, args, concurrency):
    ...
    if not full_attack_map: console.print("[green][✔] No promising candidates found. Scan complete.[/green]"); return
    ...
    for key, (db_type, result_details) in full_attack_map.items():
        ...
        if point_type == "Header":
            for method in METHODS_TO_TEST:
                ...
                if collab_host and db_type in OOB_SQLI_PAYLOADS:
                    for payload_template in OOB_SQLI_PAYLOADS[db_type]:
                        oob_id = generate_oob_id(); payload = payload_template.format(oob_id=oob_id, collab_url=collab_host, val=""); full_attack_cases.append((url, method, payload, "Header", point_name, db_type, "OOB", oob_id, oob_log_file))
        else:
            ...
            if collab_host and db_type in OOB_SQLI_PAYLOADS:
                for payload_template in OOB_SQLI_PAYLOADS[db_type]:
                    oob_id = generate_oob_id(); payload = payload_template.format(oob_id=oob_id, collab_url=collab_host, val=original_value); full_attack_cases.extend([(url,"GET",payload,"Parameter",point_name,db_type,"OOB",oob_id,oob_log_file),(url,"GET",payload,"Parameter-HPP",point_name,db_type,"OOB",oob_id,oob_log_file)])
    
    if skip_current_target.is_set(): console.print("[bold yellow]Skipping exhaustive SQLi scan due to interrupt.[/bold yellow]"); skip_current_target.clear(); return
...
def run_nuclei(gf_dir, nuclei_dir, basefile, nuclei_findings_file):
    console.print(f"\n[bold cyan]Step 6: Nuclei Scanning for Redirects...[/]")
    pattern = "redirect"
...
def main():
    parser = argparse.ArgumentParser(description="P1-URLs Scanner - v2.1.2")
    ...
    bulk_urls_output = output_dir / "target_bulk_urls.txt"
    ...
    if args.use_urls:
        workflow = [("Deduplicating URLs...", lambda: run_uro(str(input_file), deduped_urls_path)),
                    ...
                    ("Redirect Scanning...", lambda: run_nuclei(gf_dir, nuclei_dir, basefile, nuclei_file))]
    else:
    ...
    for i, (name, func) in enumerate(workflow, 1):
        ...
        func()
        if skip_current_target.is_set():
            console.print(f"[bold yellow]Step '{name}' was interrupted. Moving to next step.[/bold yellow]"); skip_current_target.clear()
